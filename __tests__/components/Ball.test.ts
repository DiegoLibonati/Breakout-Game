...
describe("Ball", () => {
  ...
  describe("Styling", () => {
    it("should only have ball class", () => {
      const container = renderComponent({ x: 0, y: 0 });
  ...
  describe("Return Type", () => {
    it("should return BallComponent type", () => {
      const container = Ball({ x: 0, y: 0 });
  ...
  describe("Multiple Balls", () => {
    it("should render multiple balls independently", () => {
      const ball1 = renderComponent({ x: 10, y: 20 });
      const ball2 = renderComponent({ x: 30, y: 40 });

      expect(ball1.style.left).toBe("10px");
      expect(ball1.style.bottom).toBe("20px");
      expect(ball2.style.left).toBe("30px");
      expect(ball2.style.bottom).toBe("40px");
    });

    it("should have unique positions", () => {
      const ball1 = renderComponent({ x: 100, y: 100 });
      const ball2 = renderComponent({ x: 200, y: 200 });

      expect(ball1.style.left).not.toBe(ball2.style.left);
      expect(ball1.style.bottom).not.toBe(ball2.style.bottom);
    });

    it("should all have the same CSS class", () => {
      const ball1 = renderComponent({ x: 0, y: 0 });
      const ball2 = renderComponent({ x: 50, y: 50 });
      const ball3 = renderComponent({ x: 100, y: 100 });

      expect(ball1).toHaveClass("ball");
      expect(ball2).toHaveClass("ball");
      expect(ball3).toHaveClass("ball");
    });
  });
